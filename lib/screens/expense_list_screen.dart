...
class _ExpenseListScreenState extends State<ExpenseListScreen> {
  ...
  Future<void> _editExpense(Expense expense) async {
    ...
    final updated = await showAddExpenseDialog(context, opts, initial: expense);
    if (updated != null) {
      final idx = expenses.indexWhere((e) => e.id == updated.id);
      if (idx != -1) {
        expenses[idx] = updated;
        await SharedPrefsStorage.saveAll(expenses);
        if (mounted) setState(() {});
      }
    }
  }
  ...
  Future<void> _deleteExpense(Expense expense) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Hapus "${expense.title}"?'),
        actions: [
    ...
    if (ok == true) {
      await SharedPrefsStorage.deleteById(expense.id);
